...
The canonical remote-tracking ref must be freshly fetched before an authority result is relied on. CI fetches `main` into `refs/remotes/origin/main` immediately before preflight. An implementation branch cannot create, accept, or authorize its own plan or record: both files must first integrate through the owner-reviewed governance path on canonical `main`, then the implementation branch must descend from the exact authority-source commit.
...
Authorization is effective only while the exact record and accepted plan remain active at trusted canonical `main` and unchanged in the implementation state. Revocation or supersession requires a governance change that updates or removes the trusted record before further implementation proceeds.
