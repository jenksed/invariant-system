...
**ID:** P0  
**Goal:** establish Project identity, constraints, documentation, Elixir structure, CI, work governance, domain semantics, Capability integration, bounded Context, protocol strategy, Git isolation, delegated-work contracts, terminal interaction, and local project-intelligence boundaries before production implementation.
...
| P0-W13 | Define approved-root local project intelligence, SQLite-first storage, structural retrieval, provenance, invalidation, and knowledge-graph criteria. | `work/p0-w13-local-project-intelligence` | In progress |
...
- Workspace, Project, Repository, Environment, Session, Task, Run, Capability, Context, Evidence, Git, delegation, Attention, interface, and recovery boundaries;
- Root, Parent, Child, Scout, Verifier, CLI, TUI, Client-state, and local-knowledge contracts;
- approved roots, reference-only instruction authority, index state, and provenance requirements;
...
Phase 0 exits after P0-W13 is accepted and the Phase 1 work-package order is reconciled.

## Phase 1 — Deterministic local execution, interaction, and intelligence kernel

**ID:** P1  
**Goal:** prove durable supervised local work, deterministic Capability selection, bounded Context, visible delegated Runs, global Attention, isolated Repository mutation, exact-state Evidence, CLI and TUI control, approved-root project intelligence, and restart recovery before an accepted live model loop.
...
- persist domain, Capability, Context, execution, delegation, Attention, Git coordination, Evidence, interface, local-knowledge, and lifecycle events in SQLite;
- reconstruct accepted state and projections after restart;
- consolidate `waiting_for_command`, expanded Attention, `kiln.interface/v0`, and `kiln.knowledge/v0` into Phase 1 validators.
...
- retrieve one bounded local-knowledge candidate without granting it instruction authority;
...
- compile Verifier Context without author confidence narrative or write Tools;
...
- prohibit user or permission waits without an Attention item in the same transaction;
...
- validate one Patch Artifact without treating it as an initial model-backed Child role;
...
- expose public `kiln.interface/v0` contracts;
...
- deduplicate replayed events and detect event gaps;
- apply backpressure without dropping lifecycle, Attention, cancellation, Evidence invalidation, result, or recovery events;
...
The deterministic CLI must support the accepted `kiln`, `start`, `resume`, `runs`, `run`, `attention`, `answer`, `approve`, `deny`, `artifacts`, `evidence`, `receipts`, and `status` surfaces with text, JSON, JSON Lines, stable exit codes, explicit destructive confirmation, revision checks, and idempotency.

The deterministic TUI prototype must include one Root, a running Scout, a blocked Child, a completed Verifier, a depth-two Child, conversation-first navigation, breadcrumb, Child cards, Run-tree overlay, global Attention, keyboard-complete control, basic mouse, explicit composer target, simulated streams, Artifact, Evidence, Receipt, stale-Evidence transition, pause, resume, cancel, responsive layouts, renderer restart, durable reconstruction, and headless tests.
...
- accept one explicit local knowledge configuration with one approved root and required excludes;
- reject indexing when no root is accepted;
- discover at least six fixture repositories with active, archived, experimental, incomplete, abandoned, clean, dirty, branch, and detached states represented;
- canonicalize paths and reject symlink escape;
- perform no source write, Git mutation, hook, build, test, package installation, service start, language-server start, or network access;
- create content hashes and Repository snapshots;
- index generic UTF-8 metadata and FTS terms for supported and unsupported languages;
- extract dependencies from at least two manifest formats;
- extract Elixir modules, functions, tests, migrations, behaviours, supervisors, and child specifications through a versioned structural adapter;
- import one explicitly approved SCIP-like fixture without generating it;
- create typed nodes, typed edges, and provenance records;
- answer one exact symbol, dependency, structural, exact text, error signature, test, migration, and verification query without a model;
- return no more than eight compact candidates with `instruction_authority: none`;
- inspect one candidate through bounded excerpts and hash verification;
- trace one candidate from query through extractor and source state;
- invalidate one changed file and reuse one unchanged file by content hash;
- reconcile one watcher overflow through a full scan;
- expose partial status when a resource limit is reached;
...
- recover Root, Parent, Child, Command, Attention, delivery, interface, and knowledge-index state after restart;
...
- preserve the last complete knowledge snapshot when a new scan fails;
- create one compact Checkpoint;
- expose all proof state through CLI and TUI projections.

### Provisional work packages

The post-P0-W13 reconciliation can replace these boundaries.

| ID | Purpose | Branch | Depends on | Status |
| --- | --- | --- | --- | --- |
| P1-W01 | Consolidate minimum domain, delegation, interface, knowledge, policy, Context, Capability, Evidence, Git, Attention, cancellation, timeout, and event contracts. | `work/p1-w01-kernel-contracts` | P0 | Replacement plan required |
| P1-W02 | Persist the append-oriented journal and rebuild Session, Run graph, Attention, delivery, interface, and knowledge-control projections. | `work/p1-w02-event-journal-projections` | P1-W01 | Replacement plan required |
...
| P1-W09 | Implement approved-root discovery, SQLite knowledge storage, generic and Elixir structural indexing, deterministic retrieval, invalidation, and knowledge CLI inspection. | `work/p1-w09-local-project-intelligence` | P1-W01, P1-W02, P1-W05, P1-W06, P1-W07 | New package proposed |
| P1-W10 | Execute the complete Phase 1 acceptance scenario, including local knowledge retrieval, independent verification, projected merge, approval, integration, Receipt, interface recovery, and restart proof. | `work/p1-w10-phase-proof` | P1-W06 through P1-W09 | Replacement scenario required |
...
- scheduler, Worker leases, state transitions, Attention, cancellation, and crash recovery;
- fake Scout and Verifier Workers;
- Context compilation and Capability grants;
- Repository and Git adapters;
- worktree isolation and exact-state Evidence;
- CLI envelopes and exit codes;
- ExRatatui dependency review and renderer boundary;
- TUI navigation, composer, layout, inspection, and headless tests;
- knowledge configuration and root approval;
- discovery, Git observation, hashing, FTS5, and snapshot publication;
- Tree-sitter dependency and grammar review;
- dependency-manifest adapters;
- SCIP-like import boundary;
- watcher adapter and reconciliation;
- knowledge query, ranking, progressive inspection, and provenance;
...
Kiln can create, execute, delegate, inspect, interrupt, cancel, isolate, verify, integrate, restart, reconstruct, navigate, authorize, compile, retrieve, index, invalidate, externalize, and accurately report one manual Project, Session, Run graph, global Attention flow, supervised Command, isolated mutation, Evidence set, projected merge, integration decision, Receipt, Checkpoint, CLI flow, TUI flow, approved knowledge root, reference Repository corpus, structural query, and provenance trace without a live model, MCP server, remote API, Context7, embeddings, graph database, browser automation, Phoenix, ACP, AG-UI, or remote hosting provider.
...
- one model-backed Scout and one model-backed Verifier after deterministic semantics pass;
...
- persistent model, Tool, Attention, delivery, interface, and knowledge-query events;
- token, cost, Context, and per-Run accounting;
- Privacy-policy evaluation before egress of knowledge candidates;
...
Kiln completes one small Repository change through a provider-backed Root Run, uses bounded local knowledge, a Scout, and an independent Verifier, preserves authority and Git isolation, survives Client reconnect, and reports current Evidence, provenance, accounting, failures, warnings, and unresolved work through CLI and TUI.
...
- Capability, Context, delegation, Git, interface, knowledge, and integration provenance;
- independent Verifier Runs for material Claims;
- projected-merge Evidence;
- unresolved-failure and blocker reporting;
- completion readiness;
- precise proposed, applied, executed, verified, accepted, integrated, and delivered labels;
- token cost by accepted Change set.

### Phase 3 exit

A passing test becomes stale after a relevant change, and Kiln refuses to treat it as current. A final Receipt identifies the tested state, Verifier Evidence, integration decision, knowledge candidates used, and unresolved proof gaps. CLI and TUI projections agree.

## Phase 4 — Context, knowledge quality, and recovery
...
- knowledge ranking evaluation, freshness, and provenance quality;
- preference candidates that remain separate from active decisions;
- token and Context observability;
- Checkpoints, interruption summaries, and traceable compaction;
- recovery of Git ownership, policy, Capability, Context, Claims, Evidence, Steward state, knowledge snapshots, interface snapshots, and Client cursors.

Embeddings can be evaluated only after deterministic local retrieval has a measured missed-query class.
...
- crash isolation;
- conformance tests that preserve core, interface, and knowledge semantics;
...
- Project, Session, Task, and Run views;
- model and Tool streams;
- global Attention and permissions;
...
- local knowledge root, scan, candidate, and provenance inspection without a graph dashboard;
- reconnect without terminating the runtime;
- Client-local focus;
- reuse of accepted public projection and input-intent contracts.

Phoenix must not fork Run, Attention, permission, Evidence, knowledge, or navigation semantics from CLI and TUI.

## Phase 7 — TypeScript SDK

Required:

- typed Kiln-native Tool, Resource, Capability, interface, and knowledge contracts;
- cancellation and progress;
- compatibility checks;
- adapter mapping helpers;
- normalized result and Artifact-reference helpers;
- test helpers;
- example extensions and adapters.

## Pending roadmap reconciliation

P0-W04 through P0-W13 constrain the product but do not finalize implementation proof order.
...
- run Repository code to improve initial indexing;
- use embeddings before exact, structural, and FTS retrieval are evaluated;
