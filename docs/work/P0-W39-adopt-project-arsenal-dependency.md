...
- **P0-W39-R13:** The P1-S01 aggregate slice gate MUST preserve its required prose-quality component while excluding `.claude/dependencies/**` from Kiln-owned Vale validation, because materialized third-party reference content is not Kiln-authored prose. This is a verification-boundary correction only and MUST NOT modify P1-S01 runtime implementation, acceptance semantics, or authority.
...
10. Extend the `scripts/gates/slice-01` prose-quality invocation to use the same `.claude/dependencies` exclusion as direct repository prose validation so the aggregate gate does not apply Kiln-owned prose rules to pinned reference-repository content. This is verification-boundary maintenance, not P1-S01 runtime work, and not P1-S02 work.
...
| `scripts/gates/slice-01` | Extend the aggregate prose-validation glob to exclude `.claude/dependencies/**` while preserving the required prose gate (R13 requirement) |
...
- **P0-W39-AC03**
  ...
  - **Evidence:** verifier output plus ten required negative-case categories exercised through thirteen concrete negative invocations/subcases (submodule not initialized; superproject gitlink SHA differs from approved pin; materialized submodule HEAD differs from approved pin; wrong canonical URL and wrong `.gitmodules` `path`; missing or invalid lockfile; wrong plan digest; wrong `repository-truth` package digest; broken or redirected skill symlink; missing or non-executable verifier when the validator runs)
...
- **P0-W39-AC08**
  - **Given** Project Arsenal is materialized under `.claude/dependencies/project-arsenal`
  - **When** the P1-S01 aggregate slice gate runs its prose-quality component
  - **Then** Kiln-owned prose is still validated, `.claude/dependencies/**` is excluded from Vale, and the aggregate slice gate remains required and passes without treating Project Arsenal reference content as Kiln-owned prose
  - **Evidence:** exact `scripts/gates/slice-01` diff showing the prose-quality invocation extended to `vale --glob='!{deps,_build,.claude/dependencies}/**' .` while still invoked as a `run_required_tool prose_quality` component, plus successful execution of the aggregate gate on the actual integration PR head (recorded in GitHub metadata, not inside this tracked file).
...
| P0-W39-E08 | P0-W39-AC08 | `scripts/gates/slice-01` invokes the prose-quality component as `run_required_tool prose_quality 'project prose satisfies the accepted style rules' vale vale --glob='!{deps,_build,.claude/dependencies}/**' .` — the prose gate remains a required `run_required_tool` component (it is not weakened to advisory, not skipped, and not removed), and its Vale glob now excludes the pinned Project Arsenal submodule. The gate continues to assert Kiln-owned prose against the accepted style rules. Exact integration-head CI is required to prove that the aggregate gate passes after this change; the run ID and conclusion live in GitHub PR metadata and the owner-facing PR body, not inside this tracked file. |
...
- `scripts/gates/slice-01` (aggregate prose gate extended to exclude the pinned submodule content)
...
**Negative-test results:** see P0-W39-E03 above for ten required negative-case categories exercised through thirteen concrete negative invocations/subcases. Each exits non-zero with an actionable message and restores the working tree.
