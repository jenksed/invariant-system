defmodule Kiln.MinimaxM3Adapter do
  ...
  - `http_options` set to `body_format: :binary` so the body is received as a
    single binary we can size-check before any further accumulation.
  ...
  defp dispatch_bounded(request, credential, event_callback) do
    ...
    with :ok <- verify_canonical_digest(request, canonical_request),
         {:ok, bounded_body} <- perform_bounded_dispatch(request, credential),
         :ok <- enforce_bounded_receipt(bounded_body) do
      event_callback.(%{
        status: :ok,
        body: bounded_body,
        request_digest: Canonical.digest(request.schema, canonical_request)
      })
  ...
  defp verify_canonical_digest(request, canonical_request) do
  ...
  defp perform_bounded_dispatch(request, credential) do
    ...
    case safe_dispatch(dispatch, http_opts) do
      ...
      {:ok, {_status_line, _headers, body}} when is_binary(body) ->
        # 1xx / 3xx / 4xx / 5xx with binary body — still bound by byte ceiling below
        {:ok, body}
  ...
  defp safe_dispatch(dispatch, http_opts) do
    try do
      ...
      kind, _ -> {:error, CandidateInvocation.terminal_result(:E_UNKNOWN)}
    end
  end
