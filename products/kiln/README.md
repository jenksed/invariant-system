...
### Two-track qualification note (2026-08-20)

Candidate A0 (`0c6ed3a`) and candidate B0 (`5e7b013`) both contain the durable
Session journal, recovery, daemon/RPC, governed human decision, verification,
review, and bounded provider seams. Their real Loadout→Kiln→Temper integration
path passed, but neither candidate passed Kiln's full test gate. B0 additionally
contains canonical Graph/Why structures; the current Elixir Temper consumer
imports this product as source, which is not an accepted cross-product
interface. Treat all two-track promotion claims as pending the gates in
[`../../docs/qualification/two-track-qualification.md`](../../docs/qualification/two-track-qualification.md).
